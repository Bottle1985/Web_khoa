...
function ktqt_landing_enqueue_assets() {
    ...
    wp_enqueue_style(
        'ktqt-landing-style',
        get_stylesheet_uri(),
        array( 'ktqt-landing-google-fonts' ),
        wp_get_theme()->get( 'Version' )
    );
}
add_action( 'wp_enqueue_scripts', 'ktqt_landing_enqueue_assets' );

function ktqt_landing_body_classes( $classes ) {
    $classes[] = 'ktqt-landing';
    return $classes;
}
add_filter( 'body_class', 'ktqt_landing_body_classes' );
